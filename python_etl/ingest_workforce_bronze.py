...
TZ_MAP = {
    ...
    "UK": "Europe/London", "DE": "Europe/Berlin", "NL": "Europe/Amsterdam", "SE": "Europe/Stockholm",
    "UAE": "Asia/Dubai", "SA": "Asia/Riyadh", "QA": "Asia/Qatar",
    "PK": "Asia/Karachi", "IN": "Asia/Kolkata", "SG": "Asia/Singapore", "AU": "Australia/Sydney"
}
...
CITY_MAP = {
    "USA": "New York", "CA": "Toronto", "UK": "London", "DE": "Berlin", "NL": "Amsterdam", "SE": "Stockholm",
    "UAE": "Dubai", "SA": "Riyadh", "QA": "Doha", "PK": "Lahore", "IN": "Bangalore", "SG": "Singapore", "AU": "Sydney"
}

HOLIDAYS_MMDD = {
    "USA": ["01-01", "07-04", "11-28", "12-25"], "UK":  ["01-01", "05-06", "12-25", "12-26"],
    "UAE": ["01-01", "04-10", "12-02"], "PK":  ["02-05", "03-23", "08-14", "12-25"],
    "IN": ["01-26", "08-15", "10-02"], "SA": ["02-22", "09-23"], "CA": ["01-01", "07-01", "12-25"],
    "DE": ["01-01", "05-01", "10-03", "12-25"], "SG": ["01-01", "08-09", "12-25"], "AU": ["01-01", "01-26", "12-25"]
}
...
def apply_hybrid(val, category):
    ...
    maps = {
        "country": {"USA": ["usa", "U.S.A", "United States", "us"], "UK": ["uk", "U.K.", "United Kingdom"], "UAE": ["uae", "U.A.E"], "PK": ["pk", "Pakistan"], "IN": ["india", "ind"], "SA": ["ksa", "Saudi Arabia"], "CA": ["canada", "can"], "DE": ["germany", "deu"], "SG": ["singapore", "sg"], "AU": ["australia", "aus"], "NL": ["netherlands"], "SE": ["sweden"], "QA": ["qatar"]},
        "currency": {"USD": ["usd", "$"], "GBP": ["gbp", "£"], "AED": ["aed"], "PKR": ["pkr", "Rs"], "INR": ["inr", "₹"], "EUR": ["eur", "€"], "CAD": ["cad"], "SAR": ["sar"], "QAR": ["qar"], "SGD": ["sgd"], "AUD": ["aud"], "SEK": ["sek"]},
        "emp_type": {"Full-time": ["full time", "FTE", "ft"], "Contractor": ["contract", "CWR"]},
        "status": {"Active": ["active", "ACT"], "Completed": ["done", "Closed"]},
        "proj_type": {"Fixed": ["fixed price", "F.P"], "T&M": ["Time & Material", "t&m"]},
        "segment": {"B2B": ["b2b", "Business to Business", "Corporate"], "B2C": ["b2c", "Consumer"], "B2G": ["b2g", "Govt", "Government"]},
        "priority": {"Critical": ["P0", "critical"], "High": ["P1", "high"], "Medium": ["P2", "med"], "Low": ["P3", "low"]}
    }
...
def seed_enterprise_dimensions():
    ...
    mgr_ids = [f"MGR-{i:03}" for i in range(1, 81)] 
    
    projects = {}
    for j in range(1, 401):
        cli_id = random.choice(list(clients.keys()))
        projects[f"PRJ-{str(j).zfill(3)}"] = {
            "Name": f"{fake.catch_phrase()} Implementation", "Client_ID": cli_id,
            "Proj_Type": random.choice(["Fixed", "T&M"]), "Status": "Active",
            "Priority": random.choices(["Critical", "High", "Medium", "Low"], weights=[0.1, 0.3, 0.4, 0.2])[0],
            "Contract_Val": random.randint(50000, 1000000), "Skill": random.choice(list(BILLING_MATRIX.keys())),
            "Start": fake.date_between(start_date="-3y", end_date="-1y"),
            "End": fake.date_between(start_date="today", end_date="+1y"), "PM_ID": random.choice(mgr_ids)
        }
    ...
    for i in range(1, 5001): 
        ...
        dept = random.choice(["Engineering", "Data Science", "Cloud", "Analytics"])
        
        employees[f"SYS-{str(i).zfill(5)}"] = {
            "Name": fake.name(), "Designation": random.choice(["Data Analyst", "Data Engineer", "QA Engineer"]),
            "Seniority": random.choice(["Junior", "Senior", "Lead"]), "Emp_Type": random.choice(["Full-time", "Contractor"]),
            "Dept": dept, "Cost_Center": f"CC-{dept[:3].upper()}-{reg}", 
            "Skill": random.choice(list(BILLING_MATRIX.keys())), "Reg": reg,
            "City": CITY_MAP[reg], "TZ": TZ_MAP[reg], "Currency": CURRENCY_MAP[reg], 
            "Experience": random.randint(1, 12), "Mgr_ID": random.choice(mgr_ids), 
            "Join": fake.date_between(start_date="-3y", end_date="-1y"),
            "Sal": random.randint(40000, 250000), "Status": "Active"
        }
    ...
    while curr_dt <= end_dt:
        ...
        for eid, emp in emp_db.items():
            ...
            if p_id == "BENCH":
                ...
                c_name, c_ind, c_size, c_ctry, c_reg, c_seg = "Internal", "Internal", "Enterprise", emp["Reg"], REGION_MAP[emp["Reg"]], "Internal"
                task_cat, proj_phase = "Leave/Training" if is_holiday else "Internal Training", "N/A"
            else:
                proj = proj_db[p_id]
                cli = clients[proj["Client_ID"]]
                client_id = proj["Client_ID"]
                p_name, p_type, p_status, pm_id, p_pri = proj["Name"], proj["Proj_Type"], proj["Status"], proj["PM_ID"], proj["Priority"]
                c_name, c_ind, c_size, c_ctry, c_reg, c_seg = cli["Name"], cli["Industry"], cli["Size"], cli["Country"], REGION_MAP[cli["Country"]], cli["Segment"]
            ...
            def build_row(h_wrk, b_hrs, nb_hrs, a_hrs, u_pct, u_cat, r_usd, c_usd, p_usd, is_corr, entry, orig_id):
                ...
                return {
                    ...
                    "seniority_level": emp["Seniority"], "employment_type": apply_hybrid(emp["Emp_Type"], "emp_type"),
                    "employment_status": apply_hybrid(emp["Status"], "status"),
                    "experience_years": emp["Experience"], "location_country": apply_hybrid(emp["Reg"], "country"),
                    "location_region": REGION_MAP[emp["Reg"]], "location_timezone": emp["TZ"], "salary_local": emp["Sal"],
                    "salary_currency": apply_hybrid(emp["Currency"], "currency"), 
                    "project_type": apply_hybrid(p_type, "proj_type"), "project_status": apply_hybrid(p_status, "status"),
                    "project_phase": proj_phase, "project_priority": apply_hybrid(p_pri, "priority"), 
                    "project_manager_id": pm_id, 
                    "client_segment": apply_hybrid(c_seg, "segment"),
                    "client_size": c_size, "client_country": apply_hybrid(c_ctry, "country"),
                    "client_region": c_reg, "revenue_type": rev_type, 
                    "hours_worked": h_wrk, "capacity_hours": cap_hrs, "billable_hours": b_hrs, "non_billable_hours": nb_hrs,
                    "available_hours": a_hrs, "utilization_pct": u_pct, "utilization_category": u_cat, 
                    "allocation_pct": alloc_pct, "hourly_rate_usd": hourly_rate, "cost_rate_local": cost_rate_local,
                    "revenue_usd": r_usd, "cost_usd": c_usd, "profit_usd": p_usd,
                    "revenue_local": round(r_usd * cur_rate, 2), "cost_local": round(c_usd * cur_rate, 2),
                    "fx_rate": cur_rate, "fx_source": fx["SRC"], "data_quality_flag": dq_flag,
                    "is_billable": is_billable, "is_holiday": is_holiday, "is_anomaly": (h_wrk < 0 or h_wrk > 12), 
                    "is_correction": is_corr, "entry_type": entry, "original_timesheet_id": orig_id,
                    "record_created_at": now_ts, "record_updated_at": now_ts, "load_timestamp": now_ts, "batch_id": RUN_ID
                }

            buffer.append(build_row(hrs_wrk, bill_hrs, nbill_hrs, avail_hrs, util_pct, util_cat, rev_usd, cost_usd, profit_usd, False, "original", None))
            ...
            if hrs_wrk > 0 and random.random() < 0.02:
                ...
                buffer.append(build_row(corr_h, cb_hrs, cnb_hrs, 0.0, 0.0, util_cat, cr_usd, cc_usd, cp_usd, True, "correction", original_id))
